const fetch = require('node-fetch');

const
    appID =
    {
        "youtube" : "880218394199220334",
        "poker" : "755827207812677713",
        "betrayal" : "773336526917861400",
        "fishing" : "814288819477020702",
        "chess" : "832012774040141894",
        "lettertile" : "879863686565621790",
        "wordsnack" : "879863976006127627",
        "doodlecrew" : "878067389634314250",
        "awkword" : "879863881349087252",
        "spellcast" : "852509694341283871",
        "checkers" : "832013003968348200"
    },
    
    appNames = ["youtube", "poker", "betrayal", "fishing", "chess", "lettertile", "wordsnack", "awkword", "doodlecrew", "spellcast", "checkers"],

    errors =
    {
        "no" :
        {
            "client" : "No client was provided !\n",
            "vc" : "No Voice Channel was provided !\n",
            "gameName" : "No Game name was provided !\n",
            "time" : "No time limit was given to the game event !\n"
        },
        
        "invalid" :
        {
            "client" : ". Invalid client !\n",
            "vc" : "Invalid Voice Chnanel !\n",
            "gameName" : "Invalid game name. Game name can only be any of the these\n" + appNames.join(' ') + "\n",
            "time" : "The time limit should be more than 0 and be in the form of a number in seconds !\n"
        },

        "exccedTime" : "The time limit cannot be more than a week, the maximum time limit is '604800 seconds'\nIf you don't want your link to get expired, you can set the 'neverExpire' parameter to true in the options of the constructor\n",
        "missingPerms" : ". The bot doesn\'t have the permission to create invites to the channel.\n"
    };

class DiscordGames
{
   /**
    * @typedef {"youtube" | "poker" | "betrayal" | "fishing" | "chess" | "lettertile" | "wordsnack" | "awkword" | "doodlecrew" | "spellcast" | "checkers"} NameOfTheGame
    * @typedef { {neverExpire: Boolean} } options
    */
   /**
    * @param {Discord.Client} Client Your Discord Client goes in here
    * @param {NameOfTheGame} GameName Name of the game goes in here
    * @param {Number} MaxDuration Maximum duration (number in seconds) of the game event goes in here.
    * @param {options=} Options
    */
    constructor(Client, GameName, MaxDuration, Options)
    {
        this.Client = Client;
        this.GameName = GameName;
        this.MaxDuration = MaxDuration;

        if (Options) { this.neverExpire = Options.neverExpire; }

        else { this.neverExpire = null; }
    };

   /**
    * @returns {Array} An array of the available games
    */
    gameNames() { return appNames.toString().split(',').join(' '); };

   /**
    * @typedef { {code: String, inviteLink: String, createdAt: Date, validTill: Date | String, guild: { ID: Number, name: String }, channel: { ID: Number, name: String }, inviter: { ID: Number, name: String }, app: { ID: Number, name: String, description: String, summary: String, maxMembers: String}} } result
    */
   /**
    * Creates an invite to play the game in a voice channel
    * @param {Discord.VoiceChannel} VoiceChannel
    * @returns {Promise<result>} A parsed JSON object { . . . . . }
    */
    play(VoiceChannel)
    {
        return new Promise((result) =>
        {
            if (!this.Client) throw new Error(errors.no.client);

            if (this.GameName)
            {
                if (!appNames.includes(this.GameName)) throw new Error(errors.invalid.gameName);
            }

            else { throw new Error(errors.no.gameName); }

            if (this.MaxDuration === undefined) throw new Error(errors.no.time);

            if (typeof(this.MaxDuration) === 'number')
            {
                if (this.MaxDuration > 7 * 24 * 60 * 60) throw new Error(errors.exccedTime);

                if (this.MaxDuration === 0) throw new Error(errors.invalid.time);
            }

            else { throw new Error(errors.invalid.time); }

            if (VoiceChannel) 
            {
                if (VoiceChannel.id === undefined) throw new Error(errors.invalid.vc);
            }

            else { throw new Error(errors.no.vc); }

            //

            let time = this.MaxDuration;

            if (this.neverExpire === true) { time = 0; }

            const body_data = { max_age: time, target_application_id: appID[this.GameName], target_type: 2, temporary: false };
    
            const body = JSON.stringify(body_data);
    
            const headers = { authorization: 'Bot ' + this.Client.token, 'content-type' : 'application/json' };
    
            const URL = 'https://discord.com/api/v8/channels/' + VoiceChannel.id + '/invites';

            //
    
            fetch.default(URL, { method: 'POST', body: body, headers: headers }).then(x => x.json()).then((data) =>
            {
                if (data.message === '401: Unauthorized') throw new Error(data.message + errors.invalidClient);

                if (data.message === 'Missing Permissions') throw new Error(data.message + errors.missingPerms);

                const finalResult =
                {
                    "code" : data.code,
                    "inviteLink" : 'https://discord.gg/' + data.code,
                    "createdAt" : new Date(data.created_at),
                    "validTill" : time === 0 ? 'This invite link will not expire automatically, because `neverExpire` was set to `true`' : new Date(data.expires_at),

                    "guild" :
                    {
                        "ID" : parseInt(data.guild.id),
                        "name" : data.guild.name
                    },

                    "channel" :
                    {
                        "ID" : parseInt(data.channel.id),
                        "name" : data.channel.name
                    },

                    "inviter" : 
                    {
                        "ID" : parseInt(data.inviter.id),
                        "name" : data.inviter.username
                    },

                    "app" :
                    {
                        "ID" : parseInt(data.target_application.id),
                        "name" : data.target_application.name,
                        "description" : data.target_application.description,
                        "summary" : data.target_application.summary,
                        "maxMembers" : data.target_application.max_participants === null ? 'null' : data.target_application.max_participants,
                    }
                };

                result(finalResult);
            });
        });
    };
};

module.exports = DiscordGames;
