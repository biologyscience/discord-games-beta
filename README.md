...
const YouTube = new DiscordGame(client, 'youtube', 10);
const Chess = new DiscordGame(client, 'chess', 1);
const LetterTile = new DiscordGame(client, 'lettertile', 5);
const WordSnack = new DiscordGame(client, 'wordsnack', 3);
const DoodleCrew = new DiscordGame(client, 'doodlecrew', 2);
...
**Do not** expect frequent changes to this package ||unless there is a new game release|| as this package is only used to test with my friends, but it works fine with everyone.  
